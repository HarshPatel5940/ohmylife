          ...
          <Card className="hover:shadow-md transition-all cursor-pointer h-full border-l-4 border-l-blue-500">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {stats?.projectsByStatus?.active || stats?.totalProjects || 0}
              </div>
              <div className="flex items-center gap-2 mt-2">
                <Progress
                  value={
                    ((stats?.projectsByStatus?.active || 0) / (stats?.totalProjects || 1)) * 100
                  }
                  className="h-1"
                />
              </div>
        ...
        <Link href="/dashboard/tasks">
          <Card className="hover:shadow-md transition-all cursor-pointer h-full border-l-4 border-l-purple-500">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-2">
                {stats?.tasksByType?.project || 0} project · {stats?.tasksByType?.personal || 0}{" "}
                personal
              </p>
            </CardContent>
          </Card>
              ...
              <div className="space-y-3">
                {stats?.upcomingDeadlines && stats.upcomingDeadlines.length > 0 ? (
                  stats.upcomingDeadlines.slice(0, 4).map((item: any) => (
                    <div
                      key={item.id}
                      className="flex items-start gap-3 p-2 rounded hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors"
                    >
