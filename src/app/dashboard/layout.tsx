...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    fetch("/api/auth/me")
      ...
      .then((data) =>
        setCurrentUser(data as { role: string; canAccessLeads: boolean; canAccessFinance: boolean })
      )
      ...
      <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-40">
        <div className="container mx-auto px-2 sm:px-4 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-3 sm:space-x-6 flex-1 min-w-0">
            <h1 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white shrink-0">
              OhMyLife
            </h1>
