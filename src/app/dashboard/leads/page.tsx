  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const leadData = {
        ...
        description,
        status,
        value: parsedValue ? Math.max(0, parsedValue) : null,
      };
          ...
          <Card className="border-l-4 border-l-green-500">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {leads.length > 0
                  ? `${((leads.filter((l) => l.status === "won").length / leads.length) * 100).toFixed(1)}% conversion`
                  : "No data"}
              </p>
            </CardContent>
          </Card>
                    ...
                    : filteredLeads.map((lead) => (
                        <TableRow key={lead.id}>
                          ...
                          <TableCell className="text-right space-x-2">
                            <Button variant="ghost" size="sm" onClick={() => openDialog(lead)}>
                        ...
                        >
                          <CardContent className="p-4 space-y-2">
                            ...
                            <div className="flex justify-between items-center text-sm text-gray-500">
                              <span>
                                {lead.value ? `₹${lead.value.toLocaleString("en-IN")}` : "No Value"}
                              </span>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <form onSubmit={handleSave}>
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="contactMode" className="text-right">
                  Contact Mode
                </Label>
                <Input
                  id="contactMode"
