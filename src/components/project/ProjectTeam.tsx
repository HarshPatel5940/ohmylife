...
export function ProjectTeam({
  ...
  const handleRemoveMember = async () => {
    ...
    setRemoving(true);
    try {
      const res = await fetch(`/api/projects/${projectId}/team/${memberToRemove}`, {
        method: "DELETE",
      });
      if (res.ok) {
        onTeamChange();
  ...
  const availablePeople = people.filter((p) => !teamMembers.some((tm) => tm.id === p.id));
        ...
        {teamMembers.length === 0 ? (
          ...
          teamMembers.map((member) => (
            ...
            >
              ...
              {currentUserRole === "admin" && (
                <Button variant="ghost" size="sm" onClick={() => openRemoveDialog(member.id)}>
      ...
      <Dialog open={addMemberOpen} onOpenChange={setAddMemberOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Add a person to this project team</DialogDescription>
          </DialogHeader>
